...
test('package manifest is configured for Local Review to Markdown publishing', () => {
  ...
  assert.equal(manifest.name, 'local-review');
  assert.equal(manifest.displayName, 'Local Review to Markdown');
  assert.equal(manifest.publisher, 'tmsdnl');
  assert.equal(manifest.private, undefined);
  assert.equal(manifest.license, 'MIT');
  assert.equal(manifest.repository?.url, 'https://github.com/tmsdnl/vscode-local-review.git');
  assert.equal(manifest.bugs?.url, 'https://github.com/tmsdnl/vscode-local-review/issues');
